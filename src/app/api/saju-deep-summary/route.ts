...
[위 '${user.ilju} 일주 해석' 블록의 traits에서 가장 인상적인 특징 2-3개를 자연스러운 산문으로 연결]을 가진 당신은 [이 특성이 만드는 구체적 재능/기질, 그림이 그려지는 문장].
...
- **일주 성격 표현**: "${user.ilju} 일주 해석" 블록의 **traits**에서 2-3개를 골라 자연스러운 산문으로. "큰 나무", "깊은 물" 같은 오행 자연물 비유 금지.
- **격국 단어 절대 금지**: "정관격", "편재격", "상관격", "식신격", "정인격", "편인격", "비견", "겁재", "건록격", "양인격" 등 격국 이름을 본문에 절대 쓰지 마세요. 괄호 안에도 쓰지 마세요. "격국 특유의", "격국의 영향으로" 같은 표현도 전부 금지. 일반 독자가 알 수 없는 명리학 용어이므로 그 의미를 풀어서 평이한 단어로만 설명하세요. (예: "정관격의 품위" → "사회적 규범과 질서를 중시하는 성향", "상관격으로 인해" → "기존 틀을 깨고 자기 방식대로 일하는 기질")
...
function buildLightPrompt(
  ...
  // Shared saju elements. We deliberately omit 격국 from the user-visible
  // label — that term is for the model's reasoning but never makes it
  // into the output (격국 단어 자체가 프롬프트에서 금지됨).
...
- **격국 단어 자체 금지.** "정관격", "편재격", "상관격", "식신격", "정인격", "편인격", "비견", "겁재", "건록격", "양인격", "격국" 등 명리학 용어를 본문에 절대 쓰지 마세요. 일반 독자가 모르는 단어이므로 의미를 풀어서 평이한 말로만 표현하세요.
