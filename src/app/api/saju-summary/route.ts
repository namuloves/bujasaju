...
1) 일주·월지 풀이 (2-3문장) — ${user.ilju} 일주의 핵심 성격을 먼저. 위 "${user.ilju} 일주 해석" 데이터의 traits 키워드를 활용.

   **그 다음 월지 ${user.wolji} 풀이 (이 부분이 가장 중요)**:
   위 "# 월지: ${user.wolji}" 섹션의 **"${user.ilgan} 일간과의 관계"** 줄을 반드시 읽고 그 내용을 그대로 paraphrase 할 것. 그 줄은 보통 "X 식신/상관의 기운" "Y 편재의 기운" 같은 십신 작용이 적혀 있음. **이걸 한국어 풀이로 풀어쓰기**:
   - ✅ 좋은 예 (사월지·갑일간): "월지가 사라서 식신·상관 기운이 강합니다. 자기 재능을 밖으로 표현하는 데 강하고, 창작이나 기획 쪽에서 두각을 드러내요."
   - ❌ 나쁜 예: "월지 사는 갑의 성격을 보완해줍니다" (데이터 무시·구체성 0)
   - ❌ 나쁜 예: "월지가 깊이 있는 안정성을 더해줍니다" (빈말)

   월지 풀이는 **데이터 줄에 적힌 십신·오행 작용을 반드시 명시**해야 함. "보완", "조화", "더해주는" 같은 추상 표현 절대 금지. "큰 나무", "깊은 물" 같은 오행 자연물 비유 금지.

2) 격국 특성 (1-2문장) — ${user.gyeokguk}이 어떤 성격과 재물 패턴을 만드는지. 명리학 용어를 문장 안에 자연스럽게 풀어쓸 것. 괄호 병기 금지. 예: ❌ "정관격(정직한 관리자의 성향)은 체계적인 재물 관리를 의미합니다" → ✅ "정관격이라 체계적이고 안정적인 재물 관리에 강합니다. 조직 안에서 신뢰받는 자리에 자주 가요."

3) 오행 약점 + 보완 (1문장, 또는 생략) — 부족한 오행이 명백히 있을 때만 한 문장. 보완 조언은 **추상적인 "물 관련 활동", "환경 조성" 같은 표현 절대 금지**. 대신 구체적인 영역 한 가지 — 예: ❌ "물과 관련된 활동을 하세요" → ✅ "직업 선택 시 유통이나 무역처럼 흐름이 있는 분야가 잘 맞아요" 또는 "검은색·파란색 계열을 일상에 자주 두면 흐름이 풀립니다". 모호한 조언밖에 안 나올 것 같으면 이 부분 통째로 생략.

### 절대 금지
- **부자 이름 언급 금지.** 이 풀이는 순수하게 사용자의 사주만 다룹니다. 매칭된 부자와의 연결은 별도 섹션에서 합니다.
- **7문장 이상 금지.** 전체 5-6문장.
- **괄호 안 한자/명리학 용어 풀이 금지** — "정관격(정직한 관리자의 성향)", "편재(예상 밖의 재물)" 같은 형태 절대 금지. 문장 안에 자연스럽게 풀어 쓸 것. 명리학 용어 그대로 사용 OK, 단 그 뒤에 풀이가 자연스럽게 이어져야 함.
- **빈말·추상 표현 금지** — "깊이 있는", "안정성을 더해주는", "원활한 흐름", "필수적인 요소", "이끌어내는 데 필수적인" 같은 알맹이 없는 형용 절대 금지. 구체적 사실·행동·결과만 적을 것.
...
- **번역체/공문서체 절대 금지.** "제공합니다", "확대해줍니다", "부여합니다", "가져다줍니다", "선사합니다", "더해줍니다", "발휘하게 해줍니다", "이끌어내는", "필수적인 요소로 작용합니다", "재물 패턴을 만들어냅니다", "성격을 보완해줍니다", "원활한 소통 덕분에", "순조롭게 풀리는 경향", "수익을 올릴 가능성이 높습니다" 사용 금지.
- **"만들어냅니다" 패턴 자체 금지.** "X은 Y를 만들어냅니다" 구문 절대 금지. X가 어떤 사람인지 직접 묘사할 것.
- **"~경향이 있습니다" 금지.** 단정적으로 짧게 쓸 것.
- "훌륭합니다", "탁월합니다", "뛰어납니다" 같은 올드패션 칭찬 금지. "좋아요", "강해요", "많아요" 같은 평이한 표현 사용.
- **어미 — 단정형 베이스 ("~합니다 / ~입니다") + "~예요/이에요" 한 단락 1번 이하.**
  "~예요/이에요/거든요/해요" 어미 한 풀이 전체에 2번 초과 금지. 위반 시 풀이 다시 작성.
  ❌ "갑술 일주는 진취적이에요. 도전적이에요. 사교적이에요." (해요체 폭격)
  ✅ "갑술 일주는 진취적이고 도전적입니다. 활동 범위가 넓고 사교성도 강합니다."
  부드럽게 마무리하고 싶을 때만 마지막 문장에 "~예요" 살짝 한 번.
