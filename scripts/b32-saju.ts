...
const sample: B[] = JSON.parse(fs.readFileSync('/tmp/b32-sample.json', 'utf8'));
const results: any[] = [];
for (const p of sample) {
  try {
    ...
    console.log(`${p.id.padEnd(5)} ${(p.nameKo||p.name).padEnd(16)} ${p.birthday}  ${yp} ${mp} ${dp}  일주=${r.ilju} 월지=${r.wolji} 격국=${r.gyeokguk}`);
  } catch (e: any) {
    console.error(`${p.id} ERROR:`, e.message);
  }
}
fs.writeFileSync('/tmp/b32-saju.json', JSON.stringify(results, null, 2));
console.log('\nSaved → /tmp/b32-saju.json (' + results.length + ' rows)');
