// B31 Generate — 20 deep bio JSONs from /tmp/b31-sample.json + /tmp/b31-saju.json + b31-data.ts
// Output: public/deep-bios-v2/{id}.json

import * as fs from 'fs';
import * as path from 'path';
import { data as DATA } from './b31-data';
...
const samples: Sample[] = JSON.parse(fs.readFileSync('/tmp/b31-sample.json', 'utf8'));
const sajus: Saju[] = JSON.parse(fs.readFileSync('/tmp/b31-saju.json', 'utf8'));
...
const GYEOK_DESC: Record<string, string> = {
  ...
  '정인격': '본인이 외부 지식·자격·인증 자본을 본인 자리로 환원시키는 흐름. 학식과 자격이 본업의 신뢰가 되는 패턴',
...
const CATEGORY_NARRATIVE: Record<string, {
  ...
  knownForKo: (companyKo: string) => string;
  industryKo: string;
}> = {
  manufacturing: {
    ...
    knownForKo: (c) => `${c} 본업, 글로벌 제조 카테고리 자수성가 정체성`,
    industryKo: '제조업',
  },
  realEstate: {
    ...
    knownForKo: (c) => `${c} 본업, 부동산 디벨로퍼 자수성가 정체성`,
    industryKo: '부동산',
  },
  tech: {
    ...
    knownForKo: (c) => `${c} 본업, 디지털 카테고리 자수성가 정체성`,
    industryKo: '테크놀로지',
  },
  media: {
    primaryIncomeKo: (c) => `${c}의 본인 보유 지분과 사업권 가치가 자산의 핵심입니다.`,
    ...
    failuresKo: [
      { year: 2015, descKo: '디지털 전환기 본업 광고 매출이 단기 시험받았습니다.', lessonKo: '전통 미디어 매출은 플랫폼 사이클에 직접 노출됩니다.' },
      { year: 2020, descKo: '코로나 광고 시장 축소로 본업 매출이 단기 시험받았습니다.', lessonKo: '미디어 매출은 거시 경기 사이클과 직접 연결됩니다.' },
    ],
    knownForKo: (c) => `${c} 본업, 미디어 자수성가 정체성`,
    industryKo: '미디어·엔터테인먼트',
  },
  gambling: {
    primaryIncomeKo: (c) => `${c}의 본인·가족 보유 지분 가치와 배당이 자산의 핵심입니다.`,
    acceleratorsKo: '본업 정립, 슬롯·게임 IP 축적, 그리고 합병·통합 거버넌스의 세 단계가 자본 가치 가속의 축입니다.',
    advantagesKo: '게임·슬롯 카테고리 자리, 일본 파친코 자본망, 그리고 합병 거버넌스 구조가 강점입니다.',
    failuresKo: [
    ...
    knownForKo: (c) => `${c} 본업, 파친코·슬롯 카테고리 자수성가 정체성`,
    industryKo: '게임·카지노',
  },
  diversified: {
    primaryIncomeKo: (c) => `${c}의 본인·가족 보유 지분 가치와 배당이 자산의 핵심입니다.`,
    ...
    knownForKo: (c) => `${c} 본업, 다각화 가문 정체성`,
    industryKo: '다각화 그룹',
  },
  healthcare: {
    primaryIncomeKo: (c) => `${c}의 본인 보유 지분 가치와 배당이 자산의 핵심입니다.`,
    acceleratorsKo: '본업 정립, 브랜드 카테고리 확장, 그리고 자본 시장 진입의 세 단계가 자본 가치 가속의 축입니다.',
    advantagesKo: '헬스케어·뷰티 카테고리 자리, 브랜드 자본망, 그리고 유통 거버넌스 구조가 강점입니다.',
    failuresKo: [
      { year: 2018, descKo: '카테고리 경쟁 심화로 본업 브랜드 매출이 단기 시험받았습니다.', lessonKo: '브랜드 매출은 카테고리 경쟁 사이클에 직접 노출됩니다.' },
      { year: 2022, descKo: '중국 소비 둔화로 본업 매출이 단기 시험받았습니다.', lessonKo: '소비재 매출은 거시 소비 사이클과 직접 연결됩니다.' },
    ],
    knownForKo: (c) => `${c} 본업, 헬스케어·뷰티 자수성가 정체성`,
    industryKo: '헬스케어',
  },
  food: {
    primaryIncomeKo: (c) => `${c}의 본인·가족 보유 지분 가치와 배당이 자산의 핵심입니다.`,
    ...
    failuresKo: [
      { year: 2008, descKo: '중국 식품 안전 이슈기 본업 신뢰가 단기 시험받았습니다.', lessonKo: '식음료 매출은 신뢰 사이클에 가장 먼저 흔들립니다.' },
      { year: 2020, descKo: '코로나로 본업 외식 채널 매출이 단기 시험받았습니다.', lessonKo: '오프라인 식음료 매출은 외부 충격기 직접 노출됩니다.' },
    ],
    knownForKo: (c) => `${c} 본업, 식음료 자수성가 정체성`,
    industryKo: '식음료',
  },
  construction: {
    primaryIncomeKo: (c) => `${c}의 본인 보유 지분과 수주 잔고가 자산의 핵심입니다.`,
    acceleratorsKo: '본업 정립, 대형 인프라 수주 확장, 그리고 해외 진출 카테고리의 세 단계가 자본 가치 가속의 축입니다.',
    advantagesKo: '건설·엔지니어링 카테고리 자리, 수주 자본망, 그리고 프로젝트 거버넌스 구조가 강점입니다.',
    failuresKo: [
      { year: 2015, descKo: '중국 부동산 사이클 조정으로 본업 수주가 단기 시험받았습니다.', lessonKo: '건설 매출은 부동산 사이클과 직접 연결됩니다.' },
      { year: 2022, descKo: '글로벌 인프라 투자 둔화로 본업 수주가 단기 시험받았습니다.', lessonKo: '건설 수주 매출은 거시 투자 사이클에 직접 노출됩니다.' },
    ],
    knownForKo: (c) => `${c} 본업, 건설·엔지니어링 자수성가 정체성`,
    industryKo: '건설·엔지니어링',
  },
  finance: {
    ...
    knownForKo: (c) => `${c} 본업, 인도 주식 투자 정체성`,
    industryKo: '금융·투자',
  },
  automotive: {
    primaryIncomeKo: (c) => `${c}의 본인·가족 보유 지분 가치와 배당이 자산의 핵심입니다.`,
    ...
    knownForKo: (c) => `${c} 본업, 글로벌 자동차 자수성가 정체성`,
    industryKo: '자동차',
  },
  energy: {
    primaryIncomeKo: (c) => `${c}의 본인 보유 지분 가치가 자산의 핵심입니다.`,
    ...
    knownForKo: (c) => `${c} 본업, 글로벌 에너지 투자 정체성`,
    industryKo: '에너지',
  },
  telecom: {
    primaryIncomeKo: (c) => `${c} 임원 보상과 보유 자산이 자산의 핵심입니다.`,
    acceleratorsKo: '대기업 입사, 계열사 대표 자리, 그리고 그룹 대표 자리 환원의 세 단계가 경력 자본 가속의 축입니다.',
    advantagesKo: '통신·IT 카테고리 자리, 대기업 거버넌스 노하우, 그리고 전문 경영인 신뢰 자본이 강점입니다.',
    failuresKo: [
      { year: 2010, descKo: '계열사 통합·구조조정 사이클기 경영 시험을 받았습니다.', lessonKo: '대기업 전문 경영인은 그룹 사이클에 직접 노출됩니다.' },
      { year: 2022, descKo: '통신 산업 규제·경쟁 환경 변화로 경영 시험을 받았습니다.', lessonKo: '통신 매출은 규제·경쟁 사이클에 직접 노출됩니다.' },
    ],
    knownForKo: (c) => `${c} 본업, 통신·IT 전문 경영인 정체성`,
    industryKo: '통신',
  },
  metals: {
    primaryIncomeKo: (c) => `${c}의 본인·가족 보유 지분 가치와 배당이 자산의 핵심입니다.`,
    ...
    knownForKo: (c) => `${c} 본업, 금속·광업 자수성가 정체성`,
    industryKo: '금속·광업',
  },
};
...
function buildCareerTimeline(s: Sample, d: any, _saju: Saju): any[] {
  ...
  // 2) Mid expansion (10-15 years later)
...
function buildBio(s: Sample, d: any, saju: Saju): any {
  ...
  return {
    ...
    careerTimeline: buildCareerTimeline(s, d, saju),
    turningPoints: [
...
for (const s of samples) {
  ...
  const d = DATA[s.id];
  if (!saju || !d) {
    console.error(`Missing data for ${s.id} (${s.nameKo})`);
    continue;
  }
  ...
  count++;
  console.log(`OK  ${s.id.padEnd(5)} ${s.nameKo.padEnd(14)} → ${filePath}`);
}
console.log(`\nWrote ${count} bios.`);
